...
// Solution code here...
const maxInArray = (arr) => arr.reduce((a, b) => b > a ? b : a, arr[0]);
...
const checkValues = (obj, value) => {
  ...
  const ifIncludes = myValues.includes(value)? true:false;
  return ifIncludes;
};
...
const updateNumbers = (obj) => {
  ...
  const final= names.map((names,index) => `${names}: ${numbers[index]}`);
  return final;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  for(let i=0;i<arr.length;i++)
  {
    if(arr[i].name=== character){
      let valuesArr=Object.values(arr[i]);
