...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((a, b) => a+b.reduce((a, b) => b === target ? a + 1 : a, 0), 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((a, b) => a+b.reduce((a, b) => a+b, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  const filtering = (input.map(a => a.filter(a => (a % 5 === 0)))).map(a => a.filter(a => typeof a === 'number'));
  return filtering.map(a => a.map(a => Math.pow(2, a)));};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  const filtering = data.filter(a=>{
    return a.gender === 'male' || a.gender === 'female';
  });
  const names= filtering.map(a =>{
    return a.name;
  });
